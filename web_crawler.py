...
from urllib import request  # 导入库urllib中的模块request
from chardet import detect  # 导入库chardet中的模块detect
...
while True:
    ...
    while do != 0:
        try:
            url = request.Request(base_url, headers=headers)  # 爬取准备：以设置的UA进行伪装URL
            html = request.urlopen(url).read()  # 1.利用request模块中的urlopen方法打开目标URL 2.利用read方法读取URL中的内容，初始数据为十六进制编码(0x)
            break
